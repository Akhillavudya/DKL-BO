"""SNUMAT side-test — CONSTRAINED-MIN band-gap search in the visible window:
Std-GP vs frozen DKL vs live-finetune DKL vs Random.

Mirror of the C2DB experiment `scripts/exp_window_min_bo.py`, run on the 3D SNUMAT pool.
We want the LOWEST gap that still fits above a hard floor. The objective is a constrained
minimisation:
...
gap is closest to 0.7 from above. A plain min search would instead chase the materials
below 0.7 and never settle inside the band — so the search is driven by the window_min
acquisition (see dklbo/bo/acquisition.py:window_min), the posterior partial expectation
of the headroom below the ceiling
...
Metrics (all post-init; the init set is identical across methods per seed, so it cancels):
  ...
  cumul_top50      cumulative # acquired among the 50 LOWEST in-window gaps
...
Usage
    python snumat_generalization/scripts/exp_window_min_bo.py                       # 4 methods, 10 seeds
    python snumat_generalization/scripts/exp_window_min_bo.py --seeds 2 --cycles 8  # smoke test
    python snumat_generalization/scripts/exp_window_min_bo.py --no_finetune         # skip slow live-DKL
    python snumat_generalization/scripts/exp_window_min_bo.py --lo 0.7 --hi 3.0     # change the window
"""
...
def summarize_and_plot(lo: float, hi: float) -> None:
    ...
    titles = {"best_inwin_gap": f"SNUMAT best (lowest) in-window gap found ({lo}–{hi} eV) vs cycle",
              "cumul_top50": f"SNUMAT cumulative top-50 (lowest) in-window found ({lo}–{hi} eV)"}
    for metric in METRICS:
    ...
    fig.suptitle(f"SNUMAT DKL vs Standard GP — crossover ({lo}–{hi} eV window-MIN search)",
    ...
    # ── Console summary ──
    print("\n" + "=" * 64)
    print(f"SNUMAT WINDOW-MIN SEARCH ({lo}–{hi} eV) — finals (mean over seeds), "
          "★=p<0.05 vs Std-GP")
    print("=" * 64)
