# Project Extract — DKL-BO (Updated, Consolidated)

> Structured profile of the DKL-BO project for CV / portfolio use.
> Extracted from the repository (code, configs, git history, docs) and **merged with the
> author's own confirmed answers** about context, authorship, contribution, and publishability.
> Generated 2026-06-30; consolidated 2026-07-01.

---

## 0. Author-Confirmed Context (your answers)

These facts came from you directly (not the repo) and drive the CV framing:

| Question | Your answer | CV / portfolio implication |
|---|---|---|
| **Setting / supervision** | Research assistantship under a professor/lab | List under **Experience** (not just Projects) with the lab name |
| **Code contribution** | Built it all myself (CGCNN encoder + GP/DKL head + BO loop + calibration), following the paper's math — not its code | Full first-person ownership of the ML is claimable |
| **Research contribution** | Owned **experiment design**, **analysis & figures**, and **paper writing** (research question developed with supervisor) | Claim implementation + experimentation + analysis + writing |
| **Authorship** | **First author**; professor/supervisor is co-author | "First-author manuscript in preparation" |
| **Paper status** | In progress / drafting (targeting a *Digital Discovery*–class journal) | "Manuscript in preparation" |
| **Timeframe** | ~2 months of active work (**May–June 2026**); paper writing still ongoing | "May 2026 – Present" |
| **Publishability** | **Embargoed until publication** | Repo stays **private**; no public GitHub link or live demo yet |
| **Lab / professor / institution** | *NOT YET PROVIDED — you'll fill this in* | Needed for the "Research Assistant, [Lab], [University]" line |
...
- **Type:** **Research assistantship** — research output under a professor/lab (lists under *Experience*). NOT FOUND IN REPO — lab / professor / institution name (you'll fill this in).
...
**Headline research finding (the paper's thesis):** whether a learned deep-kernel representation beats handcrafted descriptors in low-data Bayesian optimisation is *governed by pre-training data size* — the frozen-DKL top-10% curve crosses the Std-GP descriptor baseline at **N ≈ 250–500 materials**; pre-trained frozen DKL then significantly beats Std-GP on discovery breadth (top-10% 39.1 vs 33.7, **p=0.0027**).

> All numbers above are sourced from in-repo CSVs and `docs/results_summary.md` / `docs/experiment_results.md` / `docs/paper_positioning.md` — this project is unusually well-quantified for a portfolio piece.

---

## 6. Role-Specific Resume Bullets

> Framing note: this was a **research assistantship**, you are **first author** and the **sole implementer**, so first-person ownership verbs ("Designed", "Implemented", "Built") are fully justified. List under *Experience* with the lab/institution name.

- **Research / ML-Scientist (strongest fit):** As research assistant and first author, designed and ran a controlled study isolating *pre-training data size* as the variable that governs when a learned deep-kernel representation beats handcrafted descriptors in low-data Bayesian optimisation — establishing the crossover point (N≈250–500 materials) with a 30-seed paired-Wilcoxon protocol; manuscript in preparation.
...
## 8. Gaps / Cleanup Needed Before Using This Publicly
...
- **Lab / institution name:** still needed for the "Research Assistant, [Lab], [University]" Experience line (and the portfolio page).
- **README rewrite required (for eventual public release):** `READme.md` reads as personal phase-tracking notes (and even references scripts like `step1_c2db_loader.py` / `step3_dkl_bo.py` that don't match the actual `scripts/01_build_dataset.py …` naming). Replace with a public-facing README: problem, headline result, architecture diagram, how-to-run, results table.
- **Naming/typo:** file is `READme.md` (non-standard casing) — rename to `README.md`.
- **Repo hygiene:** working tree has many deleted/renamed scripts and uncommitted result CSVs (`git status` shows large drift); commit or clean before any future release so the history isn't confusing.
- **Data not included:** `data/raw/` and `data/cache/` are git-ignored (correct — large files); the eventual public README should document where to obtain C2DB/SNUMAT for reproducibility.
